...
module SM4(Kin, Din, Dout, Krdy, Drdy, EncDec, RSTn, EN, CLK, BSY, Kvld, Dvld, VI);
    input  [127:0] Kin;  // khoa dau vao
    input  [127:0] Din;  // du lieu dau vao
    input  [127:0] VI;   // vector khoi tao (IV)
    output [127:0] Dout; // du lieu dau ra
    input  Krdy;         // tin hieu khoa san sang
    input  Drdy;         // tin hieu du lieu san sang
    input  EncDec;       // khong dung trong che do CTR
    input  RSTn;         // reset muc thap
    input  EN;           // cho phep mach hoat dong
    input  CLK;          // xung he thong
    output BSY;          // tin hieu ban
    output Kvld;         // khoa da mo rong xong
    output Dvld;         // du lieu ra hop le

    // khoa mo rong (32 vong)
    wire [31:0] rk_00, rk_01, rk_02, rk_03, rk_04, rk_05, rk_06, rk_07;
    wire [31:0] rk_08, rk_09, rk_10, rk_11, rk_12, rk_13, rk_14, rk_15;
    wire [31:0] rk_16, rk_17, rk_18, rk_19, rk_20, rk_21, rk_22, rk_23;
    wire [31:0] rk_24, rk_25, rk_26, rk_27, rk_28, rk_29, rk_30, rk_31;

    // keystream tao tu viec ma hoa counter
    ...
    always @(posedge CLK or negedge RSTn) begin
        if (!RSTn) begin
            ctr_reg <= 128'b0;
        end else if (Drdy && EN) begin
            if (!Dvld) begin
                ctr_reg <= VI;         // lan dau lay IV lam counter
            end else begin
                ctr_reg <= ctr_reg + 1; // tang counter moi block
            end
        end
    end
    ...
    always @(posedge CLK or negedge RSTn) begin
        if (!RSTn) begin
            ...
            data_out_reg <= Din ^ keystream; // XOR voi du lieu de tao ban ma
        end
    end

    assign Dout = data_out_reg;

    // ma hoa counter de tao keystream
    SM4_EncDec enc(
        .clk(CLK),
        .rstn(RSTn),
        .sm4_enable(EN),
        ...
        .ready_out(Dvld),
        .key_exp_out(Kvld),
        .rk_00(rk_00), .rk_01(rk_01), .rk_02(rk_02), .rk_03(rk_03),
        .rk_04(rk_04), .rk_05(rk_05), .rk_06(rk_06), .rk_07(rk_07),
        .rk_08(rk_08), .rk_09(rk_09), .rk_10(rk_10), .rk_11(rk_11),
        .rk_12(rk_12), .rk_13(rk_13), .rk_14(rk_14), .rk_15(rk_15),
        .rk_16(rk_16), .rk_17(rk_17), .rk_18(rk_18), .rk_19(rk_19),
        .rk_20(rk_20), .rk_21(rk_21), .rk_22(rk_22), .rk_23(rk_23),
        .rk_24(rk_24), .rk_25(rk_25), .rk_26(rk_26), .rk_27(rk_27),
        .rk_28(rk_28), .rk_29(rk_29), .rk_30(rk_30), .rk_31(rk_31)
    );

    // mo rong khoa tu khoa dau vao
    SM4_KeyExp k_exp(
        .clk(CLK),
        .rstn(RSTn),
        .sm4_enable(EN),
    ...
    // tin hieu ra
    assign Dout = data_out_reg;
    assign BSY = ~Dvld; // ban khi chua co du lieu
...
module SM4_KeyExp (input clk, // CLK
    ...
    assign count_for_reg = reg_count_round; // Không ??o th? t? vì ch? mã hóa
...
module SM4_EncDec (input clk, // CLK
    ...
    always@(posedge clk) begin
        ...
        if(rstn == 0)
            ...
            res_out = {word_3, word_2, word_1, word_0}; // lu?ng khoá sau khi mã hoá
    end
